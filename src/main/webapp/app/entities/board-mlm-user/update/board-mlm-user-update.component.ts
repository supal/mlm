...
export class BoardMlmUserUpdateComponent implements OnInit {
  ...
  protected updateForm(boardMlmUser: IBoardMlmUser): void {
    this.editForm.patchValue({
      ...
      board: boardMlmUser.board,
      mlmUser: boardMlmUser.mlmUser,
    });

    this.boardsSharedCollection = this.boardService.addBoardToCollectionIfMissing(this.boardsSharedCollection, boardMlmUser.board);
    this.mlmUsersSharedCollection = this.mlmUserService.addMlmUserToCollectionIfMissing(
      this.mlmUsersSharedCollection,
      boardMlmUser.mlmUser
    );
  }
  ...
  protected loadRelationshipsOptions(): void {
    ...
    this.mlmUserService
      ...
      .pipe(
        map((mlmUsers: IMlmUser[]) => this.mlmUserService.addMlmUserToCollectionIfMissing(mlmUsers, this.editForm.get('mlmUser')!.value))
      )
  ...
  protected createFromForm(): IBoardMlmUser {
    return {
      ...
      mlmUser: this.editForm.get(['mlmUser'])!.value,
    };
  }
}
